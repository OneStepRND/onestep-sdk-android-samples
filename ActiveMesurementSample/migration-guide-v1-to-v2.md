...
| **Monitoring** | Configured at init | Separate `monitoring.initialize()` |
| **Recording** | Configured at init | Separate `motionLab.initialize()` |
...
// 1. Initialize MotionLab (once, after SDK init + identify)
OneStep.motionLab.initialize(
    config = OSTMotionLabConfig(
        debug = OSTMotionLabConfig.Debug(
            mockImu = false  // set true for testing without physical movement
        )
    )
)

// 2. Optional: Pre-warm for faster start (NEW)
...
// 4. Add markers during recording (NEW)
...
| Feature | v1 | v2 |
|---------|----|----|
| **Initialization** | Automatic | `motionLab.initialize()` required |
...
// 1. Initialize monitoring (after SDK init + identify)
OneStep.monitoring.initialize(
...
```kotlin
// ✅ v2 - Initialize step bouts (if not using monitoring.initialize)
...
| Feature | v1 | v2 |
|---------|----|----|
| **Enable monitoring** | Config flag | `monitoring.initialize()` |
...
|----------|---------------|
| `enableMonitoringFeature` | `OneStep.monitoring.initialize()` |
...
| `OneStep.enableBackgroundMonitoring()` | `OneStep.monitoring.initialize()` + `OneStep.monitoring.optIn()` |
...
|----------|----------------|
| `config.enableMonitoringFeature` | `OneStep.monitoring.initialize()` |
...
| `config.enableMonitoringFeature = true` | `OneStep.monitoring.initialize()` |
...
| N/A | `OneStep.motionLab.initialize()` |
...
| v1 API | v2 API |
|--------|--------|
| `config.enableMonitoringFeature = true` | `OneStep.monitoring.initialize(config)` |
...
- [ ] Add `OneStep.motionLab.initialize()` before any recording
...
- [ ] Add `OneStep.monitoring.initialize(OSTMonitoringConfig(...))` after identify
...
- [ ] Add monitoring state observation: `monitoring.state.collect { ... }`
...
OneStep.motionLab.initialize(OSTMotionLabConfig(...))
```
